"""LoRA fallback (PLAN.md Phase 2 fallback).

Same as phase2_main.py but trains the AO LoRA along with W. The injection-
layer adapter is dropped — letting the AO LoRA itself be the place where the
new K-token format gets accommodated.
"""
import argparse
import gc
import os
...
def main():
    ...
    ap.add_argument("--lr", type=float, default=1e-4, help="LR for projector + adapter")
    ap.add_argument("--ao-lora-lr", type=float, default=1e-5, help="LR for AO LoRA training")
    ap.add_argument("--epochs", type=int, default=1)
    ap.add_argument("--no-adapter", action="store_true",
                    help="Drop the injection-layer adapter; rely on LoRA fallback alone")
    ap.add_argument("--steering-coefficient", type=float, default=1.0)
    ap.add_argument("--run-name", type=str, default="phase2_lora_fallback")
    ...
    for ds_name in TRAIN_SUBDATASETS + TEST_SUBDATASETS:
        ...
        train_split, test_split = get_classification_datapoints(
            ...
            train_examples=n_train, test_examples=n_test, random_seed=42,
        )
        if n_train > 0:
    ...
    train_td = build_multi_token_classification_data(
        train_dps, tokenizer=tokenizer, model=target_model,
